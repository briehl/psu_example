package psu_example::psu_exampleClient;
...
sub new
{
    ...
    my $self = {
	client => psu_example::psu_exampleClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a psu_example.FilterContigsParams
$output is a psu_example.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	assembly_input_ref has a value which is a psu_example.assembly_ref
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	assembly_output has a value which is a psu_example.assembly_ref
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "psu_example.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for psu_example::psu_exampleClient\n";
    }
    if ($sMajor == 0) {
        warn "psu_example::psu_exampleClient version is $svr_version. API subject to change.\n";
    }
}
...
package psu_example::psu_exampleClient::RpcClient;
